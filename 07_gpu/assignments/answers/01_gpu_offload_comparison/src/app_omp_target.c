...
int main(int argc, char **argv) {
    ...
    for (int w = 0; w < warmup; w++) {
        ...
        #pragma omp target teams distribute parallel for collapse(2) \
            map(to: a[0:nn], b[0:nn]) map(from: c[0:nn])
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                float s = 0.0f;
    ...
    for (int it = 0; it < iters; it++) {
        ...
        #pragma omp target enter data map(to: a[0:nn], b[0:nn]) map(alloc: c[0:nn])
        double th1 = omp_get_wtime();
        double tk0 = omp_get_wtime();
        #pragma omp target teams distribute parallel for collapse(2) \
            map(present, to: a[0:nn], b[0:nn]) map(present, from: c[0:nn])
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                float s = 0.0f;
                for (int k = 0; k < n; k++) s += a[(size_t)i*n+k] * b[(size_t)k*n+j];
        ...
        #pragma omp target exit data map(from: c[0:nn]) map(delete: a[0:nn], b[0:nn])
    ...
    printf("RESULT,omp_target,%d,%d,%d,%.6e,%.6e,%.3e\n",
           n, iters, warmup, sum_e2e/iters, sum_compute/iters, err);
    printf("DETAIL,omp_target,%d,h2d_s=%.6e,d2h_s=%.6e\n",

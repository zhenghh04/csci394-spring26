...
#pragma omp target teams distribute parallel for collapse(2) map(to : u[0:nn]) map(from : lap[0:nn])
    for (int i = 1; i < n - 1; ++i) {
        for (int j = 1; j < n - 1; ++j) {
            lap[idx(i, j, n)] =
    ...
    double center = lap[idx(n / 2, n / 2, n)];
    double edge = lap[idx(1, 1, n)];
    printf("OpenMP target 2D Laplacian\n");

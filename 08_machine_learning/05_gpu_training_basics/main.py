...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CPU vs GPU training comparison on MNIST")
    parser.add_argument("--device", default="auto", choices=["auto", "cpu", "cuda", "xpu", "mps"])
    parser.add_argument("--epochs", type=int, default=3, help="Training epochs")
    parser.add_argument("--batch-size", type=int, default=128, help="Batch size")
    parser.add_argument("--data-dir", default="./data", help="Dataset directory")
    parser.add_argument("--limit-train", type=int, default=4096, help="Optional cap on training samples")
    parser.add_argument("--limit-test", type=int, default=1024, help="Optional cap on test samples")
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"epoch={epoch} train_loss={total_loss / total_seen:.6f} "
            f"train_acc={total_correct / total_seen:.4f} "
            f"test_loss={test_loss:.6f} test_acc={test_acc:.4f} "
            f"epoch_time_s={epoch_end - epoch_start:.6f}"
        )

    synchronize(device)
    end = time.perf_counter()
    print(f"total_training_time_s={end - start:.6f}")

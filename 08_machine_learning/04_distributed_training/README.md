...
- `train_tiny_llm_ddp.py`
  Tiny decoder-only language model with a built-in corpus for runnable DDP
  demonstrations of next-token prediction.
- `tensor_parallel_linear_demo.py`
  Minimal tensor-parallel demo that splits one linear layer across ranks and
  shows the required all-gather and all-reduce communication.
- `run_scaling_qsub.sh`
...
- `LLM_DISTRIBUTED_TRAINING.md`
  Teaching note that extends DDP to LLM-scale training with tensor parallelism,
  pipeline parallelism, and FSDP/ZeRO.
...
Student-friendly LLM concept demos:
...
- use `LLM_DISTRIBUTED_TRAINING.md` when you want to bridge from DDP to
  real LLM training systems
- use `train_tiny_llm_ddp.py` when students need a small runnable next-token
  example before discussing full LLM stacks
- use `tensor_parallel_linear_demo.py` when introducing tensor parallelism as
  a communication pattern rather than as a full framework

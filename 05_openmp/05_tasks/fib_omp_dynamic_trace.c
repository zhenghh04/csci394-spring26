...
static void write_trace_event(FILE *f, int *first, const char *name,
    ...
    *first = 0;
    fprintf(f,
            "{\"name\":\"%s\",\"cat\":\"fib_omp_dynamic\",\"ph\":\"X\",\"ts\":%.3f,\"dur\":%.3f,\"pid\":1,\"tid\":%d}",
...
int main(void) {
    ...
    printf("fib_omp_dynamic: fib(%d)=%lld fib(%d)=%lld\n", lo, results[lo], hi, results[hi]);
    printf("fib_omp_dynamic_time_s=%.6f\n", t1 - t0);

    FILE *trace = fopen("fib_omp_dynamic_trace.json", "w");
    if (!trace) {
        fprintf(stderr, "Warning: could not open fib_omp_dynamic_trace.json for writing\n");
    } else {
        int first = 1;
        ...
        fprintf(trace, "\n],\"displayTimeUnit\":\"ms\"}\n");
        fclose(trace);
        printf("trace_file=fib_omp_dynamic_trace.json\n");
    }

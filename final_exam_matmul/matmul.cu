...
/*
 * TODO (Step 1): Complete the kernel.
 *   - row = blockIdx.y * BLK + threadIdx.y
 *   - col = blockIdx.x * BLK + threadIdx.x
...
void run(int n)
{
    ...
    /* TODO (Step 2): Allocate device memory for d_A, d_B, d_C */
    float *d_A, *d_B, *d_C;

    /* TODO (Step 3): Copy h_A and h_B from host to device */

    /* Launch configuration */
    dim3 block(BLK, BLK);
    /* TODO (Step 4): Set grid dimensions */
    dim3 grid( /* fill in */ 1, /* fill in */ 1 );
    ...
    /* TODO (Step 5): Compute achieved GFLOPS and efficiency.
    ...
    /* TODO (Step 6): Copy d_C back to h_C */

    /* TODO (Step 6): Free device memory */
